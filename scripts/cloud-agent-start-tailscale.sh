...
#
# This script must not fail the Cloud Agent boot. Missing files, a missing
# secret, or a Tailscale daemon error should leave the agent usable for
# repo work; Mac-mini SSH is best-effort.
set -uo pipefail
...
if ! "${TAILSCALE_BIN}" up \
  ...
  --accept-routes \
  --ssh=false; then
  echo "cloud-agent-start-tailscale: tailscale up failed; Cloud Agent will continue without the tailnet."
  exit 0
fi
